-- 智慧酒店管理系统演示数据
-- 警告：执行后会清空 hotel_management 中现有的 8 张业务表。
...
TRUNCATE TABLE business_metric_event;
...
-- 三笔可解释定价样例：分别展示促销让利、旺季增收和提前预订优惠。
...
-- 防超卖与幂等保护的可视化演示事件；真实请求会继续自动追加。
INSERT INTO business_metric_event
    (event_type, room_id, user_id, booking_id, detail, create_time) VALUES
('INVENTORY_CONFLICT', 2, 8, NULL, '[DEMO-METRIC-01] 同房同日期库存冲突已拦截', NOW() - INTERVAL 6 DAY),
('INVENTORY_CONFLICT', 2, 9, NULL, '[DEMO-METRIC-02] 同房同日期库存冲突已拦截', NOW() - INTERVAL 6 DAY),
('IDEMPOTENT_REPLAY', 1, 2, 1, '[DEMO-METRIC-03] 网络重试返回原订单', NOW() - INTERVAL 5 DAY),
('INVENTORY_CONFLICT', 3, 10, NULL, '[DEMO-METRIC-04] 不可售房态请求已拦截', NOW() - INTERVAL 3 DAY),
('IDEMPOTENCY_KEY_CONFLICT', 6, 2, 6, '[DEMO-METRIC-05] 同一幂等键用于不同请求', NOW() - INTERVAL 2 DAY),
('IDEMPOTENT_REPLAY', 6, 2, 6, '[DEMO-METRIC-06] 网络重试返回原订单', NOW() - INTERVAL 1 DAY),
('INVENTORY_CONFLICT', 3, 11, NULL, '[DEMO-METRIC-07] 不可售房态请求已拦截', NOW() - INTERVAL 4 HOUR);
